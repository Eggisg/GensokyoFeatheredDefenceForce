...
public class StoreManager : MonoBehaviour
{
	...
	public void Update()
	{
		...
		else
		{
			...
			if (Input.GetKey(KeyCode.E)) // Why was this set to Q...
			{
				...
				placingtower2 = false;
				placingText.SetActive(true);
				placingText.GetComponent<TextMeshProUGUI>().text = CONFIRM_PLACEMENT;
			}
		}
	}
	...
	public void ConfirmBuy()
	{
		...
		foreach (GameObject borb in toBePurchasedBirbs)
		{
			if (borb.GetComponent<GenericBirbTower>() == null)
			{
				Debug.LogError("What the fuck.");
			}

			borb.GetComponent<GenericBirbTower>().PlaceTower();
			Birbthicc.showCollider = false;
		}

		for (int i = 0; i < toBePurchasedBirbs.Count; i++)
		{
			totalCost -= cost;
		}
